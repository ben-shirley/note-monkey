...
class EMNISTModel(BaseModel):
    ...
    def predict(self, word: Word) -> str:
        value = ""
        for chunk in word.get_chunks():
            value += self._predict_chunk(chunk)
        return value

    def _predict_chunk(self, chunk: Chunk):
        if self.verbosity >= 3:
            cv2.imshow('input image', chunk.image) 

        processed_image = self._preprocess(chunk.image)
    ...
    def _preprocess(self, image):
        ...
        cropped = preprocessor.crop_image_tight(image, target_color=255)
        if self.verbosity >= 4:
            cv2.imshow('cropped image', cropped)
            cv2.waitKey(0)

        padding_size = int(cropped.shape[0] * 0.1)
        padded_image = (np.zeros((cropped.shape[0] + 2*padding_size, cropped.shape[1]))).astype(np.uint8)
        ...
        squared_image = (np.zeros((padded_image.shape[0], desired_width))).astype(np.uint8)
        ...
        rotated = cv2.rotate(normalised, cv2.ROTATE_90_COUNTERCLOCKWISE)
